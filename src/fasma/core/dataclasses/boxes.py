...
class Box:
    ...
    def generate_mo_transition_analysis(self, electron: str = "alpha", zero_index=False):
        ...
        excitation_df = dfg.get_excitations_dataframe(excitation_matrix)
        summary_df = dfg.get_summary_dataframe(matrices.summarize_matrix(delta_diagonal_matrix))
    ...
    def generate_merged_mo_transition_analysis(self, zero_index=False):
        ...
        alpha_delta_mo_transition_df = dfg.get_mo_dataframe(alpha_delta_diagonal_matrix, 'AS MO ', zero_index)
        #alpha_summary_df = dfg.get_summary_dataframe(matrices.summarize_matrix(alpha_delta_diagonal_matrix))

        beta_delta_diagonal_matrix = self.spectra_data.beta_delta_diagonal_matrix
        beta_mo_transition_df = dfg.get_mo_dataframe(beta_delta_diagonal_matrix, 'Beta AS MO ', zero_index)
        #beta_summary_df = dfg.get_summary_dataframe(matrices.summarize_matrix(beta_delta_diagonal_matrix))
    ...
    def generate_ao_transition_analysis(self, electron: str = "alpha", zero_index=False):
        ...
        ao_matrix = self.pop_data.ao_matrix
        ao_matrix = matrices.convert_ao_projection_to_mo_transition(self.spectra_data.n_excitation, ao_matrix)
        ao_df = dfg.get_ao_dataframe(ao_matrix)

        excitation_matrix = self.spectra_data.excitation_matrix
        excitation_matrix = matrices.convert_mo_transition_to_ao_projection(self.basic_data.n_mo, self.spectra_data.n_excitation,  excitation_matrix)
        excitation_df = dfg.get_excitations_dataframe(excitation_matrix)
        ao_transition_matrix = self.generate_ao_transition_matrix(electron)
        summary_df = dfg.get_summary_dataframe(matrices.summarize_matrix(ao_transition_matrix))
        df = dfg.get_mo_dataframe(ao_transition_matrix, 'AS MO ', zero_index)
    ...
    def generate_ao_transition_matrix(self, electron: str = "alpha", swap_orbitals: bool = False):
        ...
        delta_diagonal_matrix = matrices.convert_mo_transition_to_ao_projection(self.basic_data.n_mo, self.spectra_data.n_excitation, delta_diagonal_matrix)
