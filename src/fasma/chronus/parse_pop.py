...
def check_pop(basic, bin_file, file_keyword_trie, file_lines) -> bool:
    ...
    try:
        ...
        if temp_check in [1, 3, 5, 7]:
            ...
            electron_data = get_alpha_electron_data(basic, file_keyword_trie, file_lines, neo_status)
            ...
            if basic.scf_type == "UHF" or basic.scf_type == "GHF":
                beta_electron_data = get_beta_electron_data(basic, file_keyword_trie, file_lines, neo_status)
...
def get_ao_matrix(basic, file_keyword_trie, file_lines):
    ...
    counter = 0
    n_lines = basic.n_mo + len(basic.atom_list) - 1
...
def get_beta_electron_data(basic, file_keyword_trie, file_lines, neo_status):
    ...
    if basic.scf_type == "UHF" or basic.scf_type == "GHF":
        mo_coefficient_matrix = get_mo_coefficient_matrix(basic=basic, file_keyword_trie=file_keyword_trie, file_lines=file_lines, beta=True, neo_status=neo_status)
...
def get_mo_coefficient_matrix(basic, file_keyword_trie, file_lines, n_col=4, neo_status=False, beta=False):
    ...
    if basic.scf_type == "GHF":
        start = file_keyword_trie.find("EigV")[index + cycles] + 1
